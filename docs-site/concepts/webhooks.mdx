...
The stock executor uses process-local endpoint, delivery-log, and queue implementations. They are suitable for development and tests, but a restart loses pending deliveries. Production self-hosters must inject durable `WebhookEndpointStore`, `WebhookDeliveryStore`, and queue implementations and add delivery metrics and alerting.
