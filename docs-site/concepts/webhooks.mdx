---
title: "Webhooks"
description: "Receive signed execution and voice events with bounded retries and project-scoped endpoints."
---

Webhooks deliver terminal execution and selected voice-session events after the source record is durable. Delivery is asynchronous: a slow or unavailable receiver never delays an execution from becoming terminal.
...
| `voice.session.event` | `voice.session.event` | Carries a selected durable `VoiceAgentSessionEvent`. The development session driver is wired; the production worker must call the same delivery seam. |
| `voice.transcript.ready` | `voice.transcript.ready` | Delivery seam is defined; production transcript publication awaits the persistent voice worker. |
...
See the [webhook API reference](/api/webhooks) for route shapes.
