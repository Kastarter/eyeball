...
Cancel pending or running work with [`POST /v1/executions/:id/cancel`](/api/executions/cancel-execution) or `eyeball.executions.cancel(executionId)`. The returned record includes `cancellation.dispatchMayHaveBegun`:
