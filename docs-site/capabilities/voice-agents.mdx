...
## Move from mock to real

Keep the agent draft, policy, allowlist, and canonical tool inputs. Replace the mock executor/worker endpoints, connect Twilio, Deepgram, ElevenLabs, Google Calendar, and Gmail for the same `userId`, remove `script`, and stop calling the development-only `/v1/dev/voice-sessions/:id/advance` route. The mock path requires zero third-party accounts.

<Warning>
  Production model-registry resolution and durable voice-worker deployment remain deployment contracts, not a hosted service implemented by this repository. Verify those pieces before treating `start_agent_call` as production-ready.
</Warning>
