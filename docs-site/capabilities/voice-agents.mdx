...
<Warning>
  The repository ships a durable control plane and deploy assets, but the account-free parity suites do not prove or claim live-call capability. The zero-database executor agent store and remote-event observer remain process-local, provider-backed recording is disabled, and the worker currently treats `llm.model` as a direct Anthropic model ID. Real-account, end-to-end certification is required before treating `start_agent_call` as production-ready.
</Warning>
