...
- configure `EYEBALL_DATABASE_URL` or provide equivalent durable execution, job, webhook-work, delivery, `FileStore`, `AgentStore`, and MCP `SessionStore` implementations;
...
- back up execution/voice history, immutable voice-agent revisions, bindings, pointers, receipts, MCP sessions/tasks, and any retained staged-file rows according to retention policy, then test restore paths;
...
7. Monitor normalized error shifts and queue state.
