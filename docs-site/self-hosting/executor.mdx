...
Postgres voice agents use stable UUID-based IDs and a mutable head that points to append-only immutable revision rows. Number bindings and executor-side session pointers reference the exact selected revision, so later agent updates cannot silently change an existing number or session. Message receipts make chat-turn replay durable. These records are passive metadata read on demand; they do not acquire leases or enter the execution/webhook job-recovery sweep.
...
<Warning>
  Postgres does not make every scheduler distributed. Trigger polling leases, rate limits, toolkit concurrency permits, and remote voice-event observation remain process-local. The current worker also requires deployment-specific queue-age/failure alerts, backup and restore drills, database-capacity monitoring and vacuum policy for potentially large `bytea` rows, and multi-replica load/chaos validation before production use. Include staged-file and voice-agent rows in backup/restore policy according to their retention requirements.
</Warning>

When `EYEBALL_DATABASE_URL` is absent, execution and webhook jobs use the same serializable state machine in memory but do not survive a process restart. Staged uploads, voice-agent definitions/revisions, number bindings, session pointers, and message receipts also remain process-local and become unavailable after restart.
