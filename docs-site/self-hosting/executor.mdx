...
<Warning>
  Postgres does not make every scheduler distributed. Trigger polling leases, rate limits, toolkit concurrency permits, and in-flight observer task handles remain process-local. Observer ownership/state and voice sources are shared and lease-fenced, but still require deployment-specific retry-age/exhaustion alerts, backup and restore drills, database-capacity monitoring and vacuum policy for potentially large `bytea` rows, and managed multi-replica load/chaos validation before production use. Include staged-file, voice-agent, observer, and voice-source rows in backup/restore policy according to their retention requirements.
</Warning>

When `EYEBALL_DATABASE_URL` is absent, execution and webhook jobs use the same serializable state machine in memory but do not survive a process restart. Staged uploads, voice-agent definitions/revisions, number bindings, session pointers, observer state, voice webhook sources, and message receipts also remain process-local and become unavailable after restart.
