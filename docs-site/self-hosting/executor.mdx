...
<Warning>
  Postgres does not make every scheduler distributed. Trigger polling leases, rate limits, toolkit concurrency permits, remote voice-event observation, and the voice-agent `AgentStore` remain process-local. The current worker also requires deployment-specific queue-age/failure alerts, backup and restore drills, and multi-replica load/chaos validation before production use.
</Warning>

When `EYEBALL_DATABASE_URL` is absent, execution and webhook jobs use the same serializable state machine in memory but do not survive a process restart.
