...
The stock server handles `SIGINT` and `SIGTERM`. It stops accepting requests and trigger polling, waits for execution and webhook queues to become idle, closes PostgreSQL persistence, and then shuts down OpenTelemetry so buffered spans and metrics can flush.
