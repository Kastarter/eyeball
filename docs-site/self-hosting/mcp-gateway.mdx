...
The default `InMemorySessionStore` and task pollers are process-local. Inject a durable `SessionStore` when negotiated sessions must survive a gateway restart; its `update` implementation must be atomic. After restart, the next authenticated request resumes polling for durable non-terminal tasks. SSE subscribers and timer handles are intentionally never persisted.

The stock `HttpMcpExecutor` allocates async work and reads execution status through the SDK. It does not advertise `tasks/cancel` because the executor has no cancellation route; a custom `McpExecutor` may implement the optional cancellation seam.
