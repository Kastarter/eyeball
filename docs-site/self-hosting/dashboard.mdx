...
The dashboard starts in **demo mode** unless cloud mode is explicitly enabled. Demo mode keeps the existing local experience and reads toolkits, executions, connections, and voice-agent data from the configured executor.
...
Cloud mode swaps only the data source for authentication, organization and project context, hosted connections, API keys, and audit events. Toolkits, executions, and voice agents continue to use the executor through the dashboard's existing server-side proxy.
...
4. Repeat for each project and browser session that needs executor-backed toolkits, executions, or voice agents.
...
<Warning>
  Cloud mode does not auto-provision or auto-inject executor credentials. The executor must already trust the pasted project key, and the dashboard and control-plane origins must be served over TLS in production.
</Warning>
