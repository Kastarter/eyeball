...
The dashboard starts in **demo mode** unless cloud mode is explicitly enabled. Demo mode keeps the existing local experience and reads toolkits, executions, connections, webhooks, triggers, and voice-agent data from the configured executor.
...
Cloud mode swaps only the data source for authentication, organization and project context, hosted connections, API keys, and audit events. Toolkits, executions, webhooks, triggers, and voice agents continue to use the executor through the dashboard's existing server-side proxy.
...
4. Repeat for each project and browser session that needs executor-backed toolkits, executions, webhooks, triggers, or voice agents.
