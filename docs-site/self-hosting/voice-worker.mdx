...
For a provider-free control-plane smoke test, start the worker with the test-only transport
explicitly enabled and no provider credentials:
...
Then create a fake session over the v2 HTTP contract, wait for its terminal state, and verify
the persisted event page has a gap-free sequence. The supplied assistant turn means this path
...
console.log({ sessionId, state, sequences });
```

This smoke test certifies only the provider-free v2 control plane. It does not certify the
Twilio, LiveKit, Deepgram, ElevenLabs, or model-provider live paths.
