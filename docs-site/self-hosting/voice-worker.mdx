...
The worker excludes the opaque grant from durable request snapshots, events, transcripts, health responses, and executor-side observer callbacks. A dedicated SQLite authorization row retains it only while needed for crash recovery and clears the bearer on terminal transition. The executor also persists grant identity, expiry, and revocation with the session pointer when `EYEBALL_DATABASE_URL` is configured. An explicit executor stop revokes immediately; a terminal event does the same when the process-local observer receives it. If an executor restart misses that terminal event, the worker has still erased its copy and the signed capability expires at its short deadline.
...
- The remote event observer is process-local, so the worker event log remains the source of truth across an executor restart. A terminal event missed during that restart can leave the executor's grant metadata unrevoked until the short signed expiry, although the terminal worker has erased its bearer. Once an observed event is admitted, its webhook-selection and delivery jobs are restart-durable when the executor uses `EYEBALL_DATABASE_URL`.
