...
When the executor supports async allocation and status reads, the initialize result advertises `tasks.requests.tools.call`. `tools/list` then adds `execution.taskSupport`: `"required"` for async-by-nature tools and `"optional"` for ordinary tools. Without this negotiation, async-by-nature tools stay listed without `execution` metadata, and a plain `tools/call` runs them through the executor's async mode and waits for the terminal record before responding.
