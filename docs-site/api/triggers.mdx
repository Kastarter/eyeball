...
`ingestUrl` appears only in the create response. Polling subscriptions omit it and include the selected `pollIntervalSeconds`.
...
Returns `204` when deleted or `404` when the subscription is absent from the authenticated project. A pinned key cannot delete another user's subscription. Deletion removes process-local cursor and dedup state and prevents future ingestion.
