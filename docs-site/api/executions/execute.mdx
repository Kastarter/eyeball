...
Async mode returns HTTP 202 with `pending`. Replaying the same async request while its execution is in flight returns HTTP 202 with the latest `pending` or `running` state. A synchronous replay waits for the original execution and returns its terminal HTTP 200 response.
