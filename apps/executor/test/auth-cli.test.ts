...
let createMockClock: MockKitModule["createMockClock"];
let createOAuthSimulation: MockKitModule["createOAuthSimulation"];
...
describe.skipIf(!mocksAvailable)(
  mocksSuiteTitle("eyeball-auth CLI", mocksAvailable),
  () => {
    beforeAll(async () => {
      ({ createMockClock, createOAuthSimulation } =
        await loadMocksModule<MockKitModule>("mock-kit"));
    });
      ...
      await runAuthCli(
        ...
        ],
        {
          ...
          prompt: async (question) => {
            ...
            codeChallenge =
              parsedAuthorizeUrl.searchParams.get("code_challenge") ??
              undefined;
