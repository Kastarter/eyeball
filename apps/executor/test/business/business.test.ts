...
type BusinessMocksModule =
  typeof import("../../../../mocks/packages/mocks-business/dist/index.js");
type BusinessHelpersModule = typeof import("./helpers.js");

let createHubSpotMock: BusinessMocksModule["createHubSpotMock"];
let createOdooMock: BusinessMocksModule["createOdooMock"];
let createQuickBooksMock: BusinessMocksModule["createQuickBooksMock"];
let createShopifyMock: BusinessMocksModule["createShopifyMock"];
let createStripeMock: BusinessMocksModule["createStripeMock"];
let createZendeskMock: BusinessMocksModule["createZendeskMock"];
let hubSpotFixtures: BusinessMocksModule["hubSpotFixtures"];
let odooFixtures: BusinessMocksModule["odooFixtures"];
let quickBooksFixtures: BusinessMocksModule["quickBooksFixtures"];
let STRIPE_CARD_DECLINE_AMOUNT: BusinessMocksModule["STRIPE_CARD_DECLINE_AMOUNT"];
let shopifyFixtures: BusinessMocksModule["shopifyFixtures"];
let stripeFixtures: BusinessMocksModule["stripeFixtures"];
let zendeskFixtures: BusinessMocksModule["zendeskFixtures"];
let createBusinessMockHarness: BusinessHelpersModule["createBusinessMockHarness"];
let executionOutput: BusinessHelpersModule["executionOutput"];
const mocksAvailable = hasMocksCheckout();

describe.skipIf(!mocksAvailable)(
  mocksSuiteTitle("P0 business adapters", mocksAvailable),
  () => {
    beforeAll(async () => {
      const [mocks, helpers] = await Promise.all([
        loadMocksModule<BusinessMocksModule>("mocks-business"),
        import("./helpers.js") as Promise<BusinessHelpersModule>,
      ]);
      ({
        createHubSpotMock,
        createOdooMock,
      ...
      ({ createBusinessMockHarness, executionOutput } = helpers);
    });
    ...
    it("runs payment, refund, customer, subscription, and invoice flows through Stripe", async () => {
      ...
      expect(
        ...
        payments: [
          expect.objectContaining({ customerId: "cus_default_000001" }),
        ],
