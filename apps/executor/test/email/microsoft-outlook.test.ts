...
let createMicrosoftOutlookMock: EmailMocksModule["createMicrosoftOutlookMock"];
let microsoftOutlookFixtures: EmailMocksModule["microsoftOutlookFixtures"];
...
describe.skipIf(!mocksAvailable)(
  mocksSuiteTitle("Microsoft Outlook email adapter", mocksAvailable),
  () => {
    beforeAll(async () => {
      ...
      ({ createMicrosoftOutlookMock, microsoftOutlookFixtures } = mocks);
    ...
    it("sends with Graph sendMail and lists the stored sent message", async () => {
      ...
      expect(storeRecords<OutlookMessage>(provider, "messages")).toHaveLength(
        1,
      );
    });
