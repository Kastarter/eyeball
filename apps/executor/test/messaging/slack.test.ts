...
let createSlackMock: MessagingMocksModule["createSlackMock"];
let slackFixtures: MessagingMocksModule["slackFixtures"];
...
describe.skipIf(!mocksAvailable)(
  mocksSuiteTitle("Slack messaging adapter", mocksAvailable),
  () => {
    beforeAll(async () => {
      ...
      ({ createSlackMock, slackFixtures } = mocks);
    ...
    it("sends, replies, reacts, reads, and discovers conversations and members", async () => {
      ...
      expect(storeRecords<SlackReaction>(provider, "reactions")).toHaveLength(
        2,
      );
