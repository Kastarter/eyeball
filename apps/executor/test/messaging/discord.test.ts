...
let createDiscordMock: MessagingMocksModule["createDiscordMock"];
let discordFixtures: MessagingMocksModule["discordFixtures"];
...
describe.skipIf(!mocksAvailable)(
  mocksSuiteTitle("Discord messaging adapter", mocksAvailable),
  () => {
    beforeAll(async () => {
      ...
      ({ createDiscordMock, discordFixtures } = mocks);
    ...
    it("executes the complete guild, message, reaction, and member route subset", async () => {
      ...
      expect(storeRecords<DiscordMessage>(provider, "messages")).toHaveLength(
        3,
      );
      expect(storeRecords<DiscordReaction>(provider, "reactions")).toHaveLength(
        2,
      );
      expect(storeRecords<DiscordChannel>(provider, "channels")).toHaveLength(
        2,
      );
    });
