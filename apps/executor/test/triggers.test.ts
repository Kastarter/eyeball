...
let createMockApp: MockKitModule["createMockApp"];
...
describe.skipIf(!mocksAvailable)(
  mocksSuiteTitle("trigger subscriptions", mocksAvailable),
  () => {
    beforeAll(async () => {
      [{ createMockApp }, { createGmailMock }] = await Promise.all([
    ...
    it("creates, lists, scopes, validates connections, and deletes subscriptions", async () => {
      ...
      await expect(otherUserList.json()).resolves.toEqual({
        subscriptions: [],
      });
    ...
    it("handles Slack URL verification and deduplicates signed push delivery", async () => {
      ...
      expect(
        Object.keys(firstHistoryPage.triggerEvents[0] ?? {}).sort(),
      ).toEqual(
        [
          "arrivalId",
          "dedupStatus",
    ...
    it("polls Gmail with a durable cursor and emits only newly received mail", async () => {
      ...
      harness.clock.advance(60_000);
      await expect(
        harness.engine.triggerService.runDue(),
      ...
      expect(
        harness.received.every(({ signatureValid }) => signatureValid),
      ).toBe(true);
      expect(
