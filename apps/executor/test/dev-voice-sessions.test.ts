...
describe("development voice-session route", () => {
  ...
  it.skipIf(!mocksAvailable)(
    mocksSuiteTitle(
      "drives a Pipecat script through its tool call and terminal transcript",
      mocksAvailable,
    ),
    async () => {
      const [
        { createMockApp, createMockClock },
        { createGmailMock },
        { createPipecatMock },
      ] = await Promise.all([
      ...
      const webhookEndpoint =
        await engine.webhookDeliverer.endpointStore.create(PROJECT_ID, {
