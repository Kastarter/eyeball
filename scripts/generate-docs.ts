...
function extensionText(manifest: ProviderManifest): string {
  const toolRows = manifest.implements.flatMap((implementation) => {
  ...
  const triggerRows = (manifest.triggers ?? []).flatMap((implementation) =>
    implementation.payloadExtensionSchema === undefined
      ? []
      : [
          `### \`${manifest.toolkit.slug}.${implementation.canonicalTrigger}\` trigger payload\n\n\`payload.x_provider.${manifest.toolkit.slug}\` may contain:\n\n\`\`\`json\n${codeJson(implementation.payloadExtensionSchema)}\n\`\`\``,
        ],
  );
  const rows = [...toolRows, ...triggerRows];
...
function toolkitPage(
  ...
  const triggerRows = triggers
    .map(
      (trigger) =>
        `| \`${trigger.name}\` | ${markdownCell(trigger.description)} | ${trigger.annotations.deliveryMode} | \`${trigger.version}\` |`,
    )
    .join("\n");
  const triggerSchemaBlocks = triggers
    .map(
      (trigger) =>
        `<Accordion title="${trigger.name}">\n\n**Payload schema**\n\n\`\`\`json\n${codeJson(trigger.payloadSchema)}\n\`\`\`\n\n</Accordion>`,
    )
    .join("\n\n");
  const effectiveScopes = [
    ...
    ...triggers.flatMap(
      (trigger) =>
        defaultCatalog.getEffectiveTriggerScopes(trigger.name)?.required ?? [],
    ),
  ]
...
description: ${JSON.stringify(`Canonical tools, triggers, auth, schemas, and mock behavior for ${manifest.toolkit.displayName}.`)}
---
...
${triggers.length === 0 ? "This manifest does not implement a canonical trigger." : `| Trigger | Purpose | Ingestion | Version |\n|---|---|---|---|\n${triggerRows}`}

${triggers.length === 0 ? "" : `Subscribe with \`eyeball.subscriptions.create\`. Trigger events arrive through the signed webhook engine as \`trigger.<toolkit>.<name>\`.`}

## Input and output schemas

${schemaBlocks}

${triggers.length === 0 ? "" : `## Trigger payload schemas\n\n${triggerSchemaBlocks}`}
...
async function main(): Promise<void> {
  ...
  const checksum = createHash("sha256")
    .update(
      JSON.stringify({
        ...
        triggers: defaultCatalog.listTriggers(),
      }),
    )
    .digest("hex")
    .slice(0, 16);
  ...
  for (const manifest of manifests) {
    ...
    const triggers = defaultCatalog.listTriggers({
    ...
    await writeDeterministic(
      ...
      toolkitPage(manifest, tools, triggers, checksum),
    );
