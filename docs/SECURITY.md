...
| SEC-030 | P2 | Open, accepted (voice not launched) | The Python voice worker loads `LIVEKIT_URL` without the `_secure_service_url()` guard applied to the executor and public URLs, so a `ws://` LiveKit endpoint would carry room/join media tokens over cleartext. | Require `wss://` outside explicit loopback for `LIVEKIT_URL`, mirroring the remote-session HTTPS rule (SEC-014); 0.5 day. |
...
| SEC-034 | P3 | Open, accepted | `scripts/check-secrets.ts` skips tracked text files larger than its size limit with a warning but exit 0, so an oversized tracked text file would bypass the offline secret gate, contradicting the "scan every existing tracked text file" requirement. No tracked text file currently exceeds the limit. | Fail closed or chunk-scan oversized tracked text files; under 0.5 day. |
