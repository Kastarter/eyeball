...
- Trigger polling still needs distributed leases, replay/backfill, provider-native signature verification, and an atomic claim/outbox. MCP sessions/tasks, voice-agent definitions, and remote voice-event observation retain documented process-local restart limits. The new Postgres execution/webhook worker still needs production backup/restore drills, queue-age and exhausted-job alerts, and multi-replica load/chaos evidence.
